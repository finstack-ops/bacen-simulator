...
- **Commitizen — REMOVED from the plan.** Commits are authored by AI agents writing [Conventional Commits](https://www.conventionalcommits.org/) directly, so no interactive commit-prompt tooling is needed. The `commitizen` / `cz-cli` dependency and the `npm run commit` workflow are dropped.
...
- **Database — [Turso](https://github.com/tursodatabase/turso) (libSQL), replacing the raw `sqlite3` driver.** The current code uses the `sqlite3` npm package with hand-rolled promisified helpers (`packages/infra/database.ts`); Turso is the decided target.
...
- [ ] **[Decided]** Remove Commitizen (`cz-cli` / `npm run commit`) — commits are authored by AI agents writing Conventional Commits directly.
...
Reference: <https://www.bcb.gov.br/content/estabilidadefinanceira/pix/changelog.html> (current DICT API version 2.12.0 as of 2026-06-29)
...
- [ ] Homologation-style test harness: scripted `pibr.001`/`002` echo tests and `pacs` payment-flow scenarios mirroring BCB's SPI/RSFN connectivity homologation regime (e.g. IN BCB 468/2024; IN BCB 508/2024 covers DICT/QR-code/payment-initiation/Pix Automático homologation specifically, not SPI message-flow tests).
