...
class TransactionManager {
	...
	private possible_transactions: {
		[key: string]: TransactionTyping.TransactionSteps;
	};
		...
		this.possible_transactions = {
			CREATE_PAYMENT: {
				...
				3: {
					...
					participants_owner: TransactionTyping.TransactionParticipants.SPI,
					participant_destiny:
						TransactionTyping.TransactionParticipants.EVERY_PSP,
				},
			},
		};
	}
	...
	continueTransaction(
		id: string,
		emitter: TransactionTyping.TransactionParticipants,
		receiver: TransactionTyping.TransactionParticipants,
		message: TransactionTyping.PossibleMessages,
	) {
		if (
			this.transactions[id].steps[
				this.transactions[id].transaction.current_step
			].message !== message ||
			this.transactions[id].steps[
				this.transactions[id].transaction.current_step
			].participant_destiny !== receiver ||
			this.transactions[id].steps[
				this.transactions[id].transaction.current_step
			].participants_owner !== emitter
		)
			throw new Error(
				'Received package doest match with expected package at this type of transaction',
			);

		this.transactions[id].transaction.current_step =
			this.transactions[id].transaction.current_step + 1;

		if (
			this.transactions[id].transaction.current_step ===
			this.transactions[id].transaction.total_steps
		) {
	...
	createTransaction(
		id: string,
		type: TransactionTyping.TransactionTypes,
		doneCallback: () => void,
		expiredCallback: () => void,
	) {
		if (this.transactions[id])
			throw new Error('Already exists an transaction with the given id.');
	...
	#getTimeoutFunction(
		id: string,
		expiredCallback: (
			transaction: TransactionTyping.RunningTransaction,
		) => void,
	) {
		return () => {
