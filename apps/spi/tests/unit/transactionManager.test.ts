...
describe('Testing transactionManager module', () => {
	...
	test('Create transaction', () => {
		...
		const transaction = manager.createTransaction(
			crypto.randomUUID(),
			TransactionTypes.CREATE_PAYMENT,
			() => {},
			() => {},
		);
	...
	test('To advance steps', () => {
		...
		let transaction: RunningTransaction | undefined = manager.createTransaction(
			id,
			TransactionTypes.CREATE_PAYMENT,
			() => {},
			() => {},
		);
		transaction = manager.continueTransaction(
			id,
			TransactionParticipants.PSP_1,
			TransactionParticipants.SPI,
			PossibleMessages.PACS_004,
		);
	...
	test('To advance all steps', () => {
		...
		manager.continueTransaction(
			id,
			TransactionParticipants.SPI,
			TransactionParticipants.EVERY_PSP,
			PossibleMessages.PACS_002,
		);
